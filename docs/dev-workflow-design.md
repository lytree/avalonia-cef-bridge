...
- `tarui.net.slnx` 为单 monorepo 解决方案，顶层分组 `src`（core/desktop/plugins/generators/webview）、`tests`、`webview/cefglue`。
...
应用侧集成：`examples/demo/Demo.Desktop.csproj` 将 `examples/demo/web/dist` 与 `examples/demo/capabilities/*.json`、capability JSON Schema 以 Content 复制进输出目录。
...
- 注册：组合根 `examples/demo/Demo.Desktop/Program.cs` 显式调用 17 个 `Add*Plugin()` 扩展方法（编译期注册，无扫描）。
...
- **合并规则**：`tarui build` / `Demo.Desktop` 构建目标将所有被引用插件的 `schema.json` 合成为应用级校验 schema（供 IDE 补全与启动期校验），与既有根 schema `schemas/tarui-desktop-capability.schema.json` 拼合。
